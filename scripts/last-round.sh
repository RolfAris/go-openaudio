#!/usr/bin/env bash
# last-round.sh — show a completed consensus round
#
# Usage:
#   ./scripts/last-round.sh [rounds_ago] [exec_cmd]
#
# rounds_ago: 0 (default) = most recently completed round
#             1 = one before that, 2 = two before, etc.
#             (skips "no proposal" rounds in the count)
#
...
#   ./scripts/last-round.sh
#
#   # Run against a Kubernetes pod
#   ./scripts/last-round.sh 0 "kubectl exec -n creator-1 audiusd-0 --"
#
#   # Run against a Docker container
#   ./scripts/last-round.sh 0 "docker exec audiusd"
#
#   # Run against a remote host via SSH
#   ./scripts/last-round.sh 0 "ssh user@node1.example.com"
#
...
# Find the target completed round, skipping back rounds_ago from the most recent.
best_round = None
best_data  = None
skipped    = 0
for r in reversed(votes):
    rnum = r.get("round", -1)
    if rnum >= cur_round:
        continue   # skip current (in-progress) round
    if skipped < rounds_ago:
        skipped += 1
        continue
    best_round = rnum
    best_data  = r
...
ago_label = "most recent" if rounds_ago == 0 else f"{rounds_ago} round(s) ago"
hdr(f"COMPLETED ROUND ({ago_label})")
if best_round is None:
    print(f"  {YELLOW}No completed rounds found in vote history{RESET}")
    sys.exit(0)

print(f"  height       : {height}")
print(f"  last round   : {best_round}  (current in-progress: {cur_round})")

# ── 2. vote breakdown ──────────────────────────────────────────────────────────

hdr(f"VOTES  (round {best_round} — final tally)")
for_l, nil_l, abs_l = tally(best_data)
...
hdr("RECENT ROUND HISTORY  (last 25 completed rounds)")
print(f"  {'Round':>6}  {'FOR':>4}  {'NIL':>4}  {'abs':>4}  note")
print(f"  {'-'*40}")
for r in votes:
    rnum = r.get("round", -1)
    if rnum >= cur_round or rnum < cur_round - 25:
        continue
    fl, nl, al = tally(r)
    gap2 = quorum - len(fl)
    note = f"{GREEN}QUORUM{RESET}" if gap2 <= 0 else (f"gap={gap2}" if len(fl) > 0 else f"{DIM}no proposal{RESET}")
    marker = "←" if rnum == best_round else " "
